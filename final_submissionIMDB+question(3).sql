...
where table_schema="imdb" AND Table_type="BASE TABLE" 
group by table_name;
...
SUM(CASE WHEN date_published IS NULL THEN 1 ELSE 0 END) AS NULL_DatePublished,
SUM(CASE WHEN duration IS NULL THEN 1 ELSE 0 END) AS NULL_Duration,
SUM(CASE WHEN country IS NULL THEN 1 ELSE 0 END) AS NULL_Country,
SUM(CASE WHEN worlwide_gross_income IS NULL THEN 1 ELSE 0 END) AS NULL_WorldWide,
...
select count(*) from movie where country like "%India%" OR country like "%USA%" and year=2019;
...
-- Type your code below:
select genre , count(genre) as count_of_movie
from genre 
group by genre
...
dense_rank () over (order by r.avg_rating )  movie_rank
from movie m 
...
SELECT nm.name AS actress_name, r.total_votes,
